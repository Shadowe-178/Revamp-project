...
public class DarkAIService {
    ...
    public ChessMoveDto darkMinimaxMove(String[][] pieces, boolean[][] revealed, String color) {
        ...
        for (ChessMoveDto move : moves) {
            ...
            int score = minimax(nextPieces, revealed, oppositeColor(color), 2, false, color);
            if (score > bestScore) {
                bestScore = score;
                bestMove = move;
            }
    ...
    private int minimax(String[][] pieces, boolean[][] revealed, String color, int depth, boolean maximizing, String rootColor) {
        if (depth == 0) {
        ...
        for (ChessMoveDto move : moves) {
            String[][] nextPieces = applyMove(pieces, move);
            int score = minimax(nextPieces, revealed, oppositeColor(color), depth - 1, !maximizing, rootColor);
            if (maximizing) {
    ...
    private int evaluateBoard(String[][] pieces, boolean[][] revealed, String color) {
        int score = 0;
        for (int y = 0; y < pieces.length; y++) {
            for (int x = 0; x < pieces[y].length; x++) {
                ...
                if (revealed[y][x]) {
                    score += (piece.startsWith(color.substring(0, 1).toLowerCase()) ? 10 : -10);
                }
            }
        }
        return score;
    ...
    private String[][] applyMove(String[][] pieces, ChessMoveDto move) {
        int[] from = parsePosition(move.getFrom());
        int[] to = parsePosition(move.getTo());
        return ruleService.applyMove(pieces, from[0], from[1], to[0], to[1]);
    }
